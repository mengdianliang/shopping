    ...
    <div class="checkout-page">
      ...
      <div class="container">
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap" v-if="addressListFilter">
            <div class="addr-list">
              <ul>
                <li :key="item.addressId"
                    v-for="item in addressListFilter"
                    :class="{'check': selectedAddr === item.addressId}"
                  ...
                  <div class="addr-opration addr-del" @click="delAddressConfirm(item)">
          ...
          <div class="next-btn-wrap">
            <router-link class="btn btn--m btn--red" :to="{path: 'orderConfirm', query: {'addressId': selectedAddr}}">Next</router-link>
          </div>
        </div>
      </div>
    </div>
      ...
      <div slot="message">
        <div class="confirm-tips">
          <div class="error-wrap">
            <span class="error error-show" v-show="errorMsg">{{errorMsg}}</span>
          </div>
          <ul>
            ...
            <li class="regi_form_input">
              <i class="icon IconAddr"></i>
              <input type="text" tabindex="2" name="streetName" v-model="streetName" class="regi_login_input regi_login_input_left" placeholder="street Name" data-type="streetName"/>
            </li>
            <li class="regi_form_input noMargin">
              <i class="icon IconPost"></i>
              <input type="text" tabindex="3" v-model="postCode" name="postCode" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="postCode"/>
            </li>
            <li class="regi_form_input noMargin">
              <i class="icon IconTel"></i>
              <input type="text" tabindex="4" v-model="tel" name="tel" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="tel"  @keyup.enter="addAddress"/>
            </li>
          </ul>
        </div>
      </div>
...
import {getAddressList, setDefault, delAddress, addAddress} from '@/api/address'
...
export default {
  ...
  methods: {
    _getAddressList () {
      getAddressList().then(res => {
        if (res.code === ERR_OK) {
          ...
          if (this.selectedAddr === -1) {
            this.selectedAddr = res.result[0].addressId
          }
        }
      })
    },
    ...
    selectAddr (item) {
      this.selectedAddr = item.addressId
    },
    setDefault (item) {
    ...
    delAddress () {
      delAddress(this.addressId).then(res => {
        if (res.code === ERR_OK) {
          // console.log(res.result)
          if (this.selectedAddr === this.addressId) {
            this.selectedAddr = -1
          }
          this.closeModal()
          this._getAddressList()
    ...
    addAddress () {
      if (!this.userName.trim() || !this.streetName.trim() || !this.postCode.trim() || !this.tel.trim()) {
