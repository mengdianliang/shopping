      ...
      <div class="cart">
        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list" v-if="cartList.length">
...
import {getCartList, delCartItem, editCartItem, checkAll} from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    delCart () {
      delCartItem(this.delItem).then(res => {
        if (res.code === ERR_OK) {
          // console.log(res)
          this.modalConfirm = false
          this._getCartList()
          this.$store.commit('updateCartCount', this.cartCount - this.delItem.productNum)
        }
      })
    },
    closeModal () {
    ...
    editCart (way, item) {
      let prevProductNum = item.productNum
      if (way === 'add') {
        item.productNum++
      } else if (way === 'minus') {
      ...
      editCartItem(item).then(res => {
        if (res.code === ERR_OK) {
          console.log(res.result)
          this.$store.commit('updateCartCount', this.cartCount + item.productNum - prevProductNum)
        }
      })
    },
    toggleCheckAll () {
